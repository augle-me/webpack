...
DepBlockHelpers.getDepBlockPromise = function(depBlock, outputOptions, requestShortener, name, extraParam = 'modelPath') {
	if(depBlock.chunks) {
		...
		if(chunks.length === 1) {
			...
			return "__webpack_require__.e" + asComment(name) + "(" + JSON.stringify(chunk.id) + "" +
				...
				asComment(depBlock.chunkReason)  + ", "+extraParam+")";
