...
class JsonpMainTemplatePlugin {
	...
	apply(mainTemplate) {
		...
		mainTemplate.plugin("jsonp-script", function(_, chunk, hash) {
			...
			const scriptSrcPath = this.applyPluginsWaterfall("asset-path", "(dynamicPath ? dynamicPath : " +JSON.stringify(chunkFilename)+")", {
		...
		mainTemplate.plugin("require-ensure", function(_, chunk, hash) {
			return this.asString([
				"if(chunkId === 0) {installedChunks[chunkId] = undefined;}",
